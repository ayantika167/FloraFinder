        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           ost =(OraclePreparedStatement) oconn.prepareStatement("SELECT * FROM USERSS where mail=?");
            ...
            if(ors.next()) 
            {
                ...
                final String username ="mitraankita613@gmail.com";
		final String password = "suctlvdhsmtfazxa";
            ...
            try {
                    ...
                    vbody += " Click the link below or copy paste in browser address bar\n\n";
                    vbody += "http://localhost:8080/ecommerce/VerifyQues.jsp?pemail="+vemail;
                    message.setText(vbody);
                    
                    Transport.send(message);
                    response.sendRedirect("http://localhost:8080/ecommerce/ForgotClose.html");
            ...
            {
                response.sendRedirect("http://localhost:8080/ecommerce/WrongEmail.html");
            }
            ost.close();
            oconn.close();
            out.println("</body>");
